...
# Global variable for storing the uploaded data
data = pd.DataFrame()
...
def process_file(uploaded_file):
    ...
    try:
        ...
        # Convert session date column to datetime format
...
def plot_data(data, value_filter):
    st.subheader(f"Visualization for {value_filter}")

    if value_filter in data.columns and not data.empty:
        fig, ax = plt.subplots(1, 2, figsize=(15, 5))

        # Bar plot
        ax[0].bar(data["Player Tag"], data[value_filter])
        ax[0].set_title(f"Bar Plot of {value_filter}")
        ax[0].set_xlabel("Player Name")
        ax[0].set_ylabel("Value")
        ax[0].tick_params(axis='x', rotation=90)

        # Line plot
        ax[1].plot(data["Player Tag"], data[value_filter], marker="o", color="blue", linestyle="-")
        ax[1].set_title(f"Line Plot of {value_filter}")
        ax[1].set_xlabel("Player Name")
...
if uploaded_file is not None:
    ...
    if data is not None:
        with st.sidebar:
            ...
            # Dynamically update filter values based on uploaded data
        ...
        value_filter = st.sidebar.selectbox("Select Value to Plot", plot_columns)

        st.subheader("Filtered Data")
        st.dataframe(aggregated_data)

        plot_data(aggregated_data, value_filter)
